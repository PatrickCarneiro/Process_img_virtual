...
const botaoComparacaoEsquerda = document.getElementById("botaoComparacaoEsquerda");
const botaoComparacaoDireita = document.getElementById("botaoComparacaoDireita");
let selecaoComparacaoDireita = "resultado";
let ladoComparacaoAtivo = "esquerda";
...
function atualizarCamposRuidoLocalvar() {

  const seletorModo =
    document.getElementById("paramModoRuidoLocalvar");

  const campoMapa =
    document.getElementById("campoMapaRuidoLocalvar");

  const campoCurva =
    document.getElementById("campoCurvaRuidoLocalvar");

  if (!seletorModo || !campoMapa || !campoCurva) return;

  if (seletorModo.value === "curva") {
    campoMapa.style.display = "none";
    campoCurva.style.display = "block";
  } else {
    campoMapa.style.display = "block";
    campoCurva.style.display = "none";
  }
}


function interpretarListaNumericaRuidoInterface(texto, nomeCampo) {
  ...
  if (
    ...
    throw new Error(
      nomeCampo + " deve conter somente valores numéricos."
    );
  }
...
function obterConfiguracaoRuidoDaInterface(nome) {
  ...
  if (nome === "Ruído Gaussiano") {
    ...
    const entradaVariancia =
      document.getElementById("paramVarianciaRuidoGaussiano");

    const media =
      entradaMedia ? entradaMedia.value.trim() : "";

    const variancia =
      entradaVariancia ? entradaVariancia.value.trim() : "";

    return criarConfiguracaoRuido(
      "gaussian",
      media,
      variancia
    );
  ...
  if (nome === "Ruído Localvar") {
    ...
    if (modo === "curva") {
      ...
      const entradaIntensidades =
        document.getElementById("paramIntensidadesRuidoLocalvar");

      const entradaVariancias =
        document.getElementById("paramVarianciasRuidoLocalvar");

      const intensidades =
        interpretarListaNumericaRuidoInterface(
          entradaIntensidades ? entradaIntensidades.value : "",
          "as intensidades do Localvar"
        );

      const variancias =
        interpretarListaNumericaRuidoInterface(
          entradaVariancias ? entradaVariancias.value : "",
          "as variâncias do Localvar"
        );
    ...
    const entradaMapa =
      document.getElementById("paramMapaVarianciaRuidoLocalvar");

    const mapaVariancia =
      interpretarListaNumericaRuidoInterface(
        entradaMapa ? entradaMapa.value : "",
        "o mapa de variância do Localvar"
      );
  ...
  throw new Error("Tipo de ruído não reconhecido.");
}


function formatarParametrosRuidoFluxograma(configuracaoEntrada) {
  ...
  if (
    ...
    return `
      Tipo: Localvar<br>
      Modo: Mapa de variância<br>
      Quantidade de valores: ${configuracao.mapaVariancia.length}
    `;
  }

  if (
    configuracao.tipo === "localvar" &&
    configuracao.modo === "curva"
  ) {
    return `
      Tipo: Localvar<br>
      Modo: Intensidade × variância<br>
      Intensidades: [${configuracao.intensidades.join(" ")}]<br>
      Variâncias: [${configuracao.variancias.join(" ")}]
    `;
  }
...
function selecionarFerramenta(nome, botaoClicado) {
  ...
  if (nome === "Equalização Convencional") {
    ...
    parametrosDiv.innerHTML = `
      ...
      <div class="campo_parametro_info">
        <label>Modo da Equalização</label>
      ...
      <div
        ...
        style="display:none;"
      >
        <label>Número de níveis discretos</label>
      ...
      <div
        ...
        style="display:none;"
      >
        <label>Valores do histograma</label>
  ...
  if (nomeEhFerramentaRuido(nome)) {
    ...
    try {
      ...
      alert(
        error && error.message
          ? error.message
          : "Não foi possível interpretar os parâmetros do ruído."
      );
      return;
    }
...
function desenharFluxograma() {
  ...
  if (modoComparativoAtivo) {
    ...
    if (String(etapaComparativoSelecionada) === "original") {
      blocoOriginal.classList.add(
        "selecionado_comparacao_esquerda"
      );
    }

    if (String(selecaoComparacaoDireita) === "original") {
      blocoOriginal.classList.add(
        "selecionado_comparacao_direita"
      );
    }

    blocoOriginal.onclick = async function() {

      await selecionarEtapaComparativo(
        "original"
      );
    };

    blocoOriginal.title =
      ladoComparacaoAtivo === "direita"
        ? "Selecionar Original para o lado direito"
        : "Selecionar Original para o lado esquerdo";
    ...
    blocoOriginal.removeAttribute("title");
  }
  ...
  pipelineFerramentas.forEach(function(etapa, index) {
    ...
    if (nomeEhFerramentaRuido(etapa.nome)) {
      ...
      try {
        textoParametros =
          formatarParametrosRuidoFluxograma(
            etapa.parametros.configuracao
          );
      } catch (error) {
    ...
    if (modoComparativoAtivo) {
      ...
      bloco.title =
        ladoComparacaoAtivo === "direita"
          ? "Selecionar esta etapa para o lado direito"
          : "Selecionar esta etapa para o lado esquerdo";
...
// Mantém válidas as duas seleções do comparativo em relação ao fluxo atual.
// O usuário escolhe primeiro qual lado deseja alterar e depois clica
// diretamente no bloco desejado do fluxograma.
function atualizarOpcoesSeletoresComparacao() {
  ...
  pipelineFerramentas.forEach(function(etapa) {

    if (!etapa || etapa.id === null || etapa.id === undefined) {
      return;
    }
...
async function toggleComparativo() {
  ...
  if (modoComparativoAtivo) {
    ...
    // Aguarda o navegador aplicar a nova largura e altura das duas áreas
    // antes de calcular a escala das imagens do comparativo.
    await new Promise(function(resolve) {
...
async function selecionarEtapaComparativo(
  ...
  if (ladoComparacaoAtivo === "direita") {
    selecaoComparacaoDireita =
      etapaId;
  } else {
    etapaComparativoSelecionada =
...
function atualizarControleLadoComparacao() {

  const esquerdaAtiva =
    ladoComparacaoAtivo !== "direita";

  if (botaoComparacaoEsquerda) {

    botaoComparacaoEsquerda.classList.toggle(
      "ativo",
      esquerdaAtiva
    );

    botaoComparacaoEsquerda.setAttribute(
      "aria-pressed",
      esquerdaAtiva
        ? "true"
        : "false"
  ...
  if (botaoComparacaoDireita) {

    botaoComparacaoDireita.classList.toggle(
      "ativo",
      !esquerdaAtiva
    );

    botaoComparacaoDireita.setAttribute(
      "aria-pressed",
      esquerdaAtiva
        ? "false"
        : "true"
...
function configurarComparacaoPeloFluxograma() {

  if (
    botaoComparacaoEsquerda &&
    botaoComparacaoEsquerda.dataset.listenerComparacao !==
      "true"
  ) {

    botaoComparacaoEsquerda.addEventListener(
      "click",
      function() {
        ...
        atualizarControleLadoComparacao();
  ...
  if (
    botaoComparacaoDireita &&
    botaoComparacaoDireita.dataset.listenerComparacao !==
      "true"
  ) {

    botaoComparacaoDireita.addEventListener(
      "click",
      function() {
...
// A legenda da esquerda acompanha a seleção azul do fluxograma.
// A legenda da direita acompanha a seleção roxa do fluxograma.
// O resultado final continua sendo a seleção inicial do lado direito.
...
if (
  ...
  const linhaVisualizacao =
    document.querySelector(".linha_visualizacao");

  if (linhaVisualizacao) {
    observadorVisualizacaoResponsiva.observe(
      linhaVisualizacao
    );
  }

  if (areaImagemOriginal) {
    observadorVisualizacaoResponsiva.observe(
      areaImagemOriginal
    );
  }

  if (areaImagemProcessada) {
    observadorVisualizacaoResponsiva.observe(
      areaImagemProcessada
    );
  }
}
...
configurarComparacaoPeloFluxograma();
